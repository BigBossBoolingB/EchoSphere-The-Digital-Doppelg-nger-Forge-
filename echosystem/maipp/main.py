...
def save_analysis(
    ...
    logger.info(
        (
            ...
            f"Inserted ID: {insert_result.inserted_id}"
        )
    )
...
def process_message(message: dict, s3_client, db_client):
    ...
    try:
        ...
        inserted_id = save_analysis(
            db_client, ingestion_event.request_id, analysis_result
        )
...
if __name__ == "__main__":
    ...
    if results:
        logger.info(
            f"Successfully processed {len(results)} messages. Inserted IDs: {results}"
        )
